...
public class JCadastro extends JFrame {
	...
	public JCadastro(Cliente clienteSelecionado, JPrincipal jPrincipal) {
		...
		textAreaEndereco.addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(KeyEvent e) {
				JButton btnNewButtonEntrar = new JButton();
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					btnNewButtonEntrar.requestFocus();
				}
			}
		});
		...
		// caso o cliente selecionado seja null ele vai incluir e caso o cliente nao
		// seja null ele vai altera.
		JButton btnNewButtonEntrar = new JButton(clienteSelecionado == null ? "INCLUIR " : "ALTERAR ");
		btnNewButtonEntrar.addKeyListener(new KeyAdapter() {
			@Override
			public void keyPressed(KeyEvent e) {
				Cliente cliente = new Cliente(null, textFieldNome.getText(), textFieldEmail.getText(),
						textFieldCpfCnpj.getText(), textFieldTelefone.getText(), textAreaEndereco.getText());
				if (clienteSelecionado == null) {
					if (!"".equalsIgnoreCase(textFieldNome.getText())
							&& !"".equalsIgnoreCase(textFieldCpfCnpj.getText())) {
		...
		btnNewButtonEntrar.setFont(new Font("Arial Black", Font.PLAIN, 11));
		btnNewButtonEntrar.setBounds(325, 0, 109, 23);
